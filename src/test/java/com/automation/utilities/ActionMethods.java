...
public class ActionMethods {
	...
	public boolean isWebElementPresent(WebElement element, WebDriver driver) {
		try {
	...
	public static Connection executeSQLQuery(String connectionName,String portNo,String SID,String uNAme,String pswd,String Query)
	    {
	    	Connection con=null;
	    	try {
				Class.forName("oracle.jdbc.driver.OracleDriver");
				String s_url="jdbc:oracle:thin:@"+connectionName+":"+portNo+":"+SID;
				con=DriverManager.getConnection(s_url,uNAme,AES.decrypt(pswd, "Ahold"));  
				Statement stmt=con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);  
				rs=stmt.executeQuery(Query);
				
			} catch (Exception e) 
	    	{
				System.out.println("SQL Query execution"+e.getMessage());
			}
	...
	public static String commaToDot(String s)
	{
		if(!isEnglish(s))
		return s.replace(",",".");
		else
			return removeCommas(s);
	}
	...
	/*public static String checkXpathExists(String x)
	{
		SetUp.driver.manage().timeouts().implicitlyWait(0,TimeUnit.SECONDS);
		String s="";
		try
		{
			if(SetUp.driver.findElements(By.xpath(x)).size() != 0)
			   s=SetUp.driver.findElement(By.xpath(x)).getText();
